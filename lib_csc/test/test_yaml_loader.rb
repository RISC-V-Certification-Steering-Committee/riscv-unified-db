...
class TestYamlLoader < Minitest::Test
  def resolve_yaml(yaml)
    Dir.mktmpdir do |dir|
      ...
      stdout, stderr, status =
        Dir.chdir($root) do
          Open3.capture3("/bin/bash -c \"source #{$root}/.home/.venv/bin/activate && #{$root}/.home/.venv/bin/python3 #{$root}/lib_csc/yaml_resolver.py resolve --no-progress --no-checks #{arch_dir} #{resolved_dir}\"")
        end
      # puts stdout
      # puts stderr
      # puts status
  ...
  def resolve_multi_yaml(*yamls)
    Dir.mktmpdir do |dir|
      ...
      system "/bin/bash -c \"source #{$root}/.home/.venv/bin/activate && #{$root}/.home/.venv/bin/python3 #{$root}/lib_csc/yaml_resolver.py resolve --no-checks #{arch_dir} #{resolved_dir}\""
      # `source #{$root}/.home/.venv/bin/activate && python3 #{$root}/lib_csc/yaml_resolver.py resolve #{arch_dir} #{resolved_dir}`

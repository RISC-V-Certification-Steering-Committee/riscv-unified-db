...
class ProcCertDesign < PortfolioDesign
  ...
  def initialize(name, arch, portfolio_design_type, proc_cert_model, proc_cert_class, overlay_path: nil)
    ...
    super(name, arch, portfolio_design_type, [proc_cert_model], proc_cert_class, overlay_path: overlay_path)
  end
  ...
  #
  # @param template_path [String] Name of template file located in backends_csc/portfolio/templates
  #                               or in backends_csc/proc_cert/templates
  ...
  def include_erb(template_name, extra_inputs = {})
    ...
    proc_cert_template_path = Pathname.new($root / "backends_csc" / proc_cert_template_pname)

    portfolio_template_pname = "portfolio/templates/#{template_name}"
    portfolio_template_path = Pathname.new($root / "backends_csc" / portfolio_template_pname)
